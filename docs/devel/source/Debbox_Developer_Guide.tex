...
Now you can use Debbox by connecting to @command{localhost:8000} 

@node Config Parser
@chapter Config Parser
Debbox provide a very easy to use and flexible configuration parser for developers. all you need to do is to build an instance of @code{Parser} 
class and pass your configuration file address to it. then you will have a dictionary like object (not all the time because some configuration files
like json type configuration files are different) that contains your configuration. 
@sp1
@quotation Note
there is some confparser driver for @code{Parser} class that allow @code{Parser} to parse specific type of configuration file, so if you pass an unkown type 
configuration file you will get a @code{Parser.TypeNotSupported} exception.
@end quotation
@sp1
@node Index
@unnumbered Index
